...
func (n *Node) Start(listenAddr string, bootstrapNodes []string) error {
	...
	if len(bootstrapNodes) > 0 {
		go n.boostrapNetwork(bootstrapNodes)
	}
...
func (n *Node) validatorLoop() {
	n.logger.Infof("starting validator loop", "pubkey", n.PrivateKey.Public(), "blockTime", blockTime)
...
func (n *Node) addPeer(c proto.NodeClient, v *proto.Version) {
	...
	if len(v.PeerList) > 0 {
		go n.boostrapNetwork(v.PeerList)
	}
...
func (n *Node) dialRemoteNode(addr string) (proto.NodeClient, *proto.Version, error) {
	...
	// other node handshake our version and returns its version
